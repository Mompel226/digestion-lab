...
(function () {
  ...
  function stickyInset(sc) {
    ...
    var inset = 0, plate = document.querySelector('.bodycol');
    if (plate && sc.contains(plate) && window.getComputedStyle(plate).position === 'sticky') inset += Math.round(plate.getBoundingClientRect().height);
    ...
    return inset + (tr.height && tr.top <= topOfScroller(sc) + tr.height + 2 ? Math.round(tr.height) : 0);
  }
  ...
  function fillSubmit() {
    ...
    if (signIn) {
      ...
      body.innerHTML = '<p class="st-sub">' + head + '</p>' + work +
        ...
        '<div class="who">Saving as <b>' + esc(signIn.name) + '</b>' +
        '<button type="button" class="tourcard__link" id="subOut">' + (stale ? 'sign in again' : 'not you?') + '</button></div>' +
  ...
  function boot() {
    ...
    if (qStat) {
      ...
      var plate = document.querySelector('.bodycol'), fold = document.getElementById('plateFold');
      if (plate && fold) {
